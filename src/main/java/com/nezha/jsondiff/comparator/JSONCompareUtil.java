...
public final class JSONCompareUtil {
    public static final double UNIQUE_KEY_WEIGHT = 1.0;
    ...
    /**
     * <p>Looks to see if unique key is a usable for both expected and actual.
     * Returns true IFF:</p>
     * <ol>
     *   <li>expected unique key val should be same as actual
     *   <li>otherwise, it makes no sense to have the unique key
     * </ol>
     *
     * @param expected a map of expected JSONArray, of key with unique key value
     * @param actual   a map of actual JSONArray, of key with unique key value
     * @return true if the same unique key value is more than UNIQUE_KEY_WEIGHT
     */
    public static boolean isUsableAsUniqueKeyForBothMap(Map<Object, JSONObject> expected,
                                                        Map<Object, JSONObject> actual) {
        int count = 0;
        for (Object keyVal : expected.keySet()) {
            if (actual.containsKey(keyVal)) {
                count++;
            }
        }
        return (double) count / expected.size() >= UNIQUE_KEY_WEIGHT;
    }
    ...
    public static List<Object> jsonArrayToList(JSONArray expected) throws JSONException {
        ...
        for (int i = 0; i < expected.length(); ++i) {
            ...
            if (o instanceof Double && (int) ((double) o) == (double) o) {
                jsonObjects.add((int) ((double) o));
            } else {
                jsonObjects.add(o);
            }
        }
    ...
    /**
     * convert to Object.NUll when object is JSONObject.NULL
     *
     * @param o the collection of items to convert
     * @return o or null
     */
    public static Object getIfNull(Object o) {
    ...
    // get param value from yaml，format: "key=key001", return key001
    public static String getParamValue(String param) {
        if (StringUtils.isNotBlank(param)) {
            return param.replaceAll(" ", "").trim().split("=")[1];
        }
