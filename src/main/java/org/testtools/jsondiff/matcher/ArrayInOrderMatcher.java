 ...
 */
public class ArrayInOrderMatcher<T> implements CustomValueMatcher<T> {
    /**
     * Create ArrayInOrderMatcher to match every JsonObject by order
     *
     */
    public ArrayInOrderMatcher() {}
    ...
    public boolean equal(String prefix, T actual, T expected, JSONCompareDetailResult result, JSONComparator comparator) {
        ...
        try {
            ...
            CompareContext mode = new CompareContext(false,true, false, false);
            comparator.compareJSONArray(mode, prefix, expectedArray, actualArray,
                    result);
