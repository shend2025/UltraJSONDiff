/*
 * ImpreciseRotateValueMatcher.java
...
/**
 * Function: inaccurate radian matcher
 */

public class RadianPreciseMatcher<T> implements ValueMatcher<T> {
    private double tolerance;

    public RadianPreciseMatcher() { this (10e-5); }

    public RadianPreciseMatcher(double tolerance) {
    ...
    public boolean equal(T actual, T expected){
        try {
            ...
            if (actualNum.doubleValue() > (2 * Math.PI) || actualNum.doubleValue() < -(2 * Math.PI)
                    || expectedNum.doubleValue() >(2 * Math.PI) || expectedNum.doubleValue() < -(2 * Math.PI)){
                return false;
            }

            Double diffAngle = (expectedNum.doubleValue() + Math.PI) % Math.PI - (actualNum.doubleValue() + Math.PI) % Math.PI;
